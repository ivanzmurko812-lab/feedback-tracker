...
function loadLocalState() {
  try {
    ...
    localState = {
      ...
      sop_docs: Array.isArray(parsed.sop_docs) ? parsed.sop_docs : [],
      activity_logs: Array.isArray(parsed.activity_logs) ? parsed.activity_logs : [],
      weekly_shift_totals: Array.isArray(parsed.weekly_shift_totals) ? parsed.weekly_shift_totals : []
    };
    ...
    const legacyUser = localState.users.find((x) => String(x.id) === "local-admin-legacy-1" || String(x.login) === "1");
    if (legacyUser) {
      legacyUser.id = "local-admin-legacy-1";
      legacyUser.login = "1";
      legacyUser.pass_hash = sha256Hex("1");
      legacyUser.role = "admin";
    } else {
      localState.users.push(createDefaultLocalState().users[1]);
    }
    saveLocalState();
  } catch (err) {
...
async function writeActivityLog({ actorLogin, action, entityType, target = "", details = "" }) {
  const row = {
    id: uuid(),
    actor_login: String(actorLogin || "").trim() || "system",
    action: String(action || "").trim() || "updated",
    entity_type: String(entityType || "").trim() || "item",
  ...
  if (LOCAL_MODE) {
    localState.activity_logs.unshift(row);
    localState.activity_logs = localState.activity_logs.slice(0, 1000);
    saveLocalState();
    return row;
  }

  const q = await pool.query(
    `INSERT INTO activity_logs(actor_login, action, entity_type, target, details, created_at)
     VALUES($1,$2,$3,$4,$5,now())
     RETURNING id, actor_login, action, entity_type, target, details, created_at`,
    [row.actor_login, row.action, row.entity_type, row.target, row.details]
  );
  return q.rows[0];
}
...
async function initDatabase() {
  ...
  await pool.query(`
    ...
    CREATE INDEX IF NOT EXISTS idx_sop_docs_uploaded_at
      ON sop_docs(uploaded_at DESC);
    ...
    CREATE TABLE IF NOT EXISTS weekly_shift_totals (
      id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
      report_date date NOT NULL,
      dp text NOT NULL,
      total_shift numeric NOT NULL DEFAULT 0,
      created_by text NOT NULL,
      updated_by text NOT NULL,
      created_at timestamptz NOT NULL DEFAULT now(),
      updated_at timestamptz NOT NULL DEFAULT now(),
      UNIQUE (report_date, dp)
    );

    CREATE INDEX IF NOT EXISTS idx_weekly_shift_totals_lookup
      ON weekly_shift_totals(report_date DESC, dp);
  `);
...
app.put("/api/users/:id", authRequired, roleRequired("admin", "manager"), async (req, res) => {
  ...
  if (LOCAL_MODE) {
    ...
    await writeActivityLog({
      ...
      entityType: "user",
      target: target.login,
      details: [role ? `role=${role}` : "", password ? "password changed" : ""].filter(Boolean).join(", ")
    });
    return res.json({ ok: true });
  }
...
app.delete("/api/users/:id", authRequired, roleRequired("admin", "manager"), async (req, res) => {
  ...
  if (LOCAL_MODE) {
    const deletedLogin = target.login || String(id);
...
app.post("/api/delivery-mark", authRequired, roleRequired("admin", "manager"), async (req, res) => {
  ...
  await writeActivityLog({
    actorLogin: req.user.login,
    action: delivered ? "marked delivered" : "marked pending",
    entityType: "delivery",
    target: `${login} / ${date}`
  });
...
app.post("/api/reports", authRequired, roleRequired("admin", "manager"), async (req, res) => {
  ...
  if (LOCAL_MODE) {
    ...
    await writeActivityLog({
      ...
      entityType: "report",
      target: `${reportType} / ${reportDate} / ${dp}`
    });
...
app.get("/api/activity-log", authRequired, async (req, res) => {
  if (LOCAL_MODE) {
    const rows = localState.activity_logs
      .slice()
      .sort((a, b) => String(b.created_at || "").localeCompare(String(a.created_at || "")));
    return res.json({ rows });
  }
  ...
  const q = await pool.query(
    `SELECT id, actor_login, action, entity_type, target, details, created_at
     FROM activity_logs
...
app.get("/api/weekly-shifts", authRequired, async (req, res) => {
  const reportDate = toPgDate(req.query.date);
  const dp = normalizeDp(req.query.dp);

  if (LOCAL_MODE) {
    const rows = localState.weekly_shift_totals
      .filter((x) => (!reportDate || x.report_date === reportDate) && (!dp || x.dp === dp))
      .sort((a, b) => String(b.report_date || "").localeCompare(String(a.report_date || "")) || String(a.dp || "").localeCompare(String(b.dp || "")));
    return res.json({ rows });
  }
  ...
  const q = await pool.query(
    `SELECT id, report_date, dp, total_shift, created_by, updated_by, created_at, updated_at
     FROM weekly_shift_totals
     ${whereSql}
     ORDER BY report_date DESC, dp ASC`,
    vals
  );
  res.json({ rows: q.rows });
});

app.post("/api/weekly-shifts", authRequired, roleRequired("admin", "manager"), async (req, res) => {
  const reportDate = toPgDate(req.body?.date);
  const dp = normalizeDp(req.body?.dp);
  const totalShift = Number(req.body?.totalShift);

  if (!reportDate || !dp || !Number.isFinite(totalShift)) {
    return res.status(400).json({ error: "missing date/dp/totalShift" });
  }

  if (LOCAL_MODE) {
    const now = new Date().toISOString();
    const idx = localState.weekly_shift_totals.findIndex((x) => x.report_date === reportDate && x.dp === dp);
    const existed = idx >= 0;
    const row = idx >= 0
      ? { ...localState.weekly_shift_totals[idx], total_shift: totalShift, updated_by: req.user.login, updated_at: now }
      : {
          id: uuid(),
          report_date: reportDate,
          dp,
    ...
    if (idx >= 0) localState.weekly_shift_totals[idx] = row;
    else localState.weekly_shift_totals.push(row);
    saveLocalState();
    await writeActivityLog({
      actorLogin: req.user.login,
      action: existed ? "updated" : "saved",
      entityType: "total shift",
      target: `${reportDate} / ${dp}`,
      details: String(totalShift)
    });
    return res.json({ ok: true, row });
  }

  const q = await pool.query(
    `INSERT INTO weekly_shift_totals(report_date, dp, total_shift, created_by, updated_by, created_at, updated_at)
     VALUES($1,$2,$3,$4,$4,now(),now())
     ON CONFLICT (report_date, dp)
     DO UPDATE SET
       total_shift = EXCLUDED.total_shift,
       updated_by = EXCLUDED.updated_by,
       updated_at = now()
     RETURNING id, report_date, dp, total_shift, created_by, updated_by, created_at, updated_at`,
    [reportDate, dp, totalShift, req.user.login]
  );
...
// SOP DOCS
app.get("/api/sop-docs", authRequired, async (req, res) => {
  if (LOCAL_MODE) {
    const rows = localState.sop_docs
      .slice()
      .sort((a, b) => String(b.uploaded_at || "").localeCompare(String(a.uploaded_at || "")))
      .map(({ id, title, file_name, mime, uploaded_by, uploaded_at }) => ({
        id,
        title,
        file_name,
        mime,
  ...
  const q = await pool.query(
    `SELECT id, title, file_name, mime, uploaded_by, uploaded_at
     FROM sop_docs
...
app.get("/api/sop-docs/:id/file", authRequired, async (req, res) => {
  const id = String(req.params.id || "").trim();
  if (!id) return res.status(400).json({ error: "missing id" });

  if (LOCAL_MODE) {
    const row = localState.sop_docs.find((x) => String(x.id) === id);
    if (!row) return res.status(404).json({ error: "not found" });
    res.setHeader("Content-Type", row.mime || "application/octet-stream");
    res.setHeader("Content-Disposition", `attachment; filename="${row.file_name || "sop.docx"}"`);
    return res.send(Buffer.from(row.bytes, "base64"));
  }

  const q = await pool.query(
    `SELECT file_name, mime, bytes FROM sop_docs WHERE id = $1`,
    [id]
  );
  const row = q.rows[0];
  if (!row) return res.status(404).json({ error: "not found" });
...
app.post("/api/sop-docs", authRequired, roleRequired("admin", "manager"), upload.single("file"), async (req, res) => {
  ...
  const row = {
    id: uuid(),
    title,
    file_name: req.file.originalname || "sop.docx",
    mime: req.file.mimetype || "application/octet-stream",
    uploaded_by: req.user.login,
    uploaded_at: new Date().toISOString()
  };

  if (LOCAL_MODE) {
    localState.sop_docs.unshift({
      ...row,
  ...
  const q = await pool.query(
    `INSERT INTO sop_docs(title, file_name, mime, bytes, uploaded_by, uploaded_at)
     VALUES($1,$2,$3,$4,$5,now())
     RETURNING id, title, file_name, mime, uploaded_by, uploaded_at`,
    [row.title, row.file_name, row.mime, req.file.buffer, row.uploaded_by]
  );
  await writeActivityLog({
    actorLogin: req.user.login,
    action: "uploaded",
...
app.delete("/api/sop-docs/:id", authRequired, roleRequired("admin", "manager"), async (req, res) => {
  const id = String(req.params.id || "").trim();
  if (!id) return res.status(400).json({ error: "missing id" });

  if (LOCAL_MODE) {
    const row = localState.sop_docs.find((x) => String(x.id) === id);
    const before = localState.sop_docs.length;
    localState.sop_docs = localState.sop_docs.filter((x) => String(x.id) !== id);
    if (localState.sop_docs.length === before) return res.status(404).json({ error: "not found" });
    saveLocalState();
    await writeActivityLog({
      actorLogin: req.user.login,
      action: "deleted",
  ...
  const cur = await pool.query(`SELECT title, file_name FROM sop_docs WHERE id = $1`, [id]);
  const q = await pool.query(`DELETE FROM sop_docs WHERE id = $1`, [id]);
  if (!q.rowCount) return res.status(404).json({ error: "not found" });
  await writeActivityLog({
    actorLogin: req.user.login,
    action: "deleted",
    entityType: "sop",
    target: cur.rows[0]?.title || id,
    details: cur.rows[0]?.file_name || ""
  });
  res.json({ ok: true });
});
...
app.delete("/api/imports/:id", authRequired, roleRequired("admin"), async (req, res) => {
  ...
  let importName = id;
  if (!LOCAL_MODE) {
    const cur = await pool.query(`SELECT name FROM imports WHERE id=$1`, [id]);
    importName = cur.rows[0]?.name || id;
  }
